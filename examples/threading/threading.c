...
void* threadfunc(void* thread_param)
{
    ...
    sleep_ret = usleep(data->wait_to_obtain_ms * USEC_IN_MSEC);

    ret = pthread_mutex_lock(data->mutex);
    
    if (ret == 0) {
        sleep_ret = usleep(data->wait_to_release_ms * USEC_IN_MSEC);
        ret = pthread_mutex_unlock(data->mutex);
    }

    data->thread_complete_success = ret || sleep_ret ? false : true;
